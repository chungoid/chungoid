...
from chungoid.schemas.common_enums import (
    StageStatus,
    FlowPauseStatus,
)  # Added FlowPauseStatus
...
def state_manager_fixture(tmp_path: Path):
    ...
    os.chdir(
        test_target_dir
    )  # Some StateManager operations might assume CWD is project root
...
class TestStateManager(unittest.TestCase):
    ...
    def test_init_success_file_does_not_exist(
        self, mock_is_dir, mock_path_exists, mock_path_open, mock_path_mkdir, mock_lock
    ):
        ...
        sm = StateManager(
            self.test_target_dir_str, server_stages_dir=self.dummy_stages_dir_str
        )
        ...
        self.assertEqual(
            mock_path_mkdir.call_args[1], {"parents": True, "exist_ok": True}
        )  # Check kwargs
        ...
        status_file_path_obj = (
            Path(self.test_target_dir_str) / ".chungoid" / "project_status.json"
        )
        ...
        self.assertFalse(
            was_called_for_writing_status_file,
            "Path.open should not have been called in 'w' mode for the status file during __init__.",
        )
    ...
    def test_init_status_file_corrupted(
        self, mock_is_dir, mock_path_exists, mock_path_open, mock_lock
    ):
        ...
        mock_path_exists.assert_has_calls(
            calls_to_exists, any_order=True
        )  # Check it was called with the path
        ...
        mock_path_open.assert_called_once_with(
            expected_status_file_path, mode="r", encoding="utf-8"
        )

    @patch.object(Path, "is_dir", return_value=False)
    def test_init_target_dir_does_not_exist(
        self, mock_is_dir
    ):  # Removed mock_class_path_open, mock_class_path_exists
    ...
    def test_init_stages_dir_does_not_exist(
        self, mock_os_mkdir, mock_path_is_dir_method
    ):  # Removed mock_class_path_open, mock_class_path_exists
        ...
        with (
            ...
            patch.object(
                StateManager, "_read_status_file", return_value={"runs": []}
            ) as mock_read_status,
        ):  # Mock _read_status_file
    ...
    def test_update_project_status(
        ...
        fixed_timestamp_dt = datetime.datetime(
            2023, 2, 15, 12, 0, 0, tzinfo=datetime.timezone.utc
        )
        ...
        def open_side_effect_for_update(
            path_instance, mode="r", encoding=None, *args, **kwargs
        ):
            if mode == "r":
        ...
        success = sm.update_status(
            stage=0.0, status=StageStatus.SUCCESS.value, artifacts=["out.log"]
        )
        ...
        expected_data_to_dump = {
            "runs": [
                {
                    "run_id": 0,
                    "start_timestamp": fixed_timestamp_dt.isoformat(),
                    "status_updates": [],
                }
            ]
        }
        ...
        mock_json_dump.assert_called_once_with(
            expected_data_to_dump, unittest.mock.ANY, indent=2
        )
        ...
        expected_calls_to_path_open = [
            call(
                expected_status_file_path, mode="r", encoding="utf-8"
            ),  # From _read_status_file (via _initialize_status or update)
            call(
                expected_status_file_path, mode="w", encoding="utf-8"
            ),  # From _write_status_file
        ]
        ...
        corrected_expected_calls = [
            call(
                "r", encoding="utf-8"
            ),  # This corresponds to the _read_status_file in update_status
            call(
                "w", encoding="utf-8"
            ),  # This corresponds to the _write_status_file in update_status
        ]
    ...
    @patch("filelock.FileLock")
    @patch.object(Path, "is_dir", return_value=True)
    def test_get_next_stage_simple(
        self, mock_is_dir, mock_lock
    ):  # Removed mock_class_path_open, mock_class_path_exists
        ...
        with (
            patch.object(sm, "_read_status_file", return_value=parsed_data),
            patch(
                "pathlib.Path.glob", return_value=[mock_stage0, mock_stage1]
            ) as mock_glob,
    ...
    def test_get_next_stage_fail(
        ...
        with (
            patch.object(sm, "_read_status_file", return_value=parsed_data),
            patch(
                "pathlib.Path.glob", return_value=[mock_stage0, mock_stage1_5]
            ) as mock_glob,
        ):  # Patch pathlib.Path.glob
    ...
    def test_get_reflection_context_from_chroma_success(
        self, mock_query_documents, mock_isdir
    ):  # Removed mock_class_path_open, mock_class_path_exists
    ...
    @patch.object(Path, "is_dir", return_value=True)
    @patch(
        "chungoid.utils.state_manager.chroma_utils.add_or_update_document"
    )  # Corrected patch target
    def test_store_artifact_context_in_chroma_success(
        self, mock_add_or_update_document, mock_isdir
    ):  # Removed mock_class_path_open, mock_class_path_exists
        ...
        with patch("filelock.FileLock"):
            ...
            success = sm.store_artifact_context_in_chroma(
                stage_number=1.0, rel_path="a/b.txt", content="artifact content"
            )
...
class TestStateManagerPauseResume(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patch_persistent_client = patch(
        ...
        self.mock_get_logger.return_value = (
            self.mock_logger
        )  # Make getLogger return our mock
        ...
        self.sm_for_pause_tests = StateManager(
            str(self.test_target_dir), server_stages_dir=str(self.dummy_stages_dir)
        )
        self.sm_for_pause_tests.status_file_path = (
            self.status_file_path_obj
        )  # Ensure it uses our Path obj
    ...
    def test_save_paused_flow_state_success(self):
        ...
        with (
            patch.object(
                ...
                return_value=self.mock_status_parent_dir,
            ) as mock_parent_prop,
            patch.object(self.mock_paused_file_path, "open", m_open),
        ):  # Patch open on the final mock file path
        ...
        self.mock_paused_runs_dir.mkdir.assert_called_once_with(
            parents=True, exist_ok=True
        )

        # Assert that the second division (mock_paused_runs_dir / f"paused_run_{paused_details.run_id}.json") was called
        self.mock_paused_runs_dir.__truediv__.assert_called_once_with(
            f"paused_run_{paused_details.run_id}.json"
        )
        ...
        self.mock_logger.info.assert_any_call(
            f"Saving paused flow state for run_id '{self.test_run_id}' to {self.mock_paused_file_path}"
        )
    ...
    def test_load_paused_flow_state_success(self):
        ...
        with patch.object(
            self.sm_for_pause_tests,
            "_get_paused_flow_file_path",
            return_value=self.mock_paused_file_path,
        ) as mock_getter:
            loaded_details = self.sm_for_pause_tests.load_paused_flow_state(
                self.test_run_id
            )
        ...
        if hasattr(loaded_details, "timestamp") and loaded_details.timestamp:
            loaded_details.timestamp = loaded_details.timestamp.astimezone(
                datetime.timezone.utc
            )
        if hasattr(paused_details_orig, "timestamp") and paused_details_orig.timestamp:
            paused_details_orig.timestamp = paused_details_orig.timestamp.astimezone(
                datetime.timezone.utc
            )
        ...
        self.assertEqual(
            loaded_details.paused_at_stage_id, paused_details_orig.paused_at_stage_id
        )
        self.assertEqual(
            loaded_details.context_snapshot, paused_details_orig.context_snapshot
        )
        self.assertEqual(
            loaded_details.status, FlowPauseStatus.PAUSED_UNKNOWN.value
        )  # Compare with the enum's value
        if loaded_details.error_details and paused_details_orig.error_details:
            self.assertEqual(
                loaded_details.error_details.message,
                paused_details_orig.error_details.message,
            )
            self.assertEqual(
                loaded_details.error_details.error_type,
                paused_details_orig.error_details.error_type,
            )
        elif (
            loaded_details.error_details != paused_details_orig.error_details
        ):  # handles if one is None and other is not
            self.fail(
                f"Error details mismatch: loaded={loaded_details.error_details}, original={paused_details_orig.error_details}"
            )
        # Compare timestamps separately due to potential precision/tz issues after deserialization
        self.assertAlmostEqual(
            loaded_details.timestamp,
            paused_details_orig.timestamp,
    ...
    def test_delete_paused_flow_state_success(self):
        ...
        self.mock_status_parent_dir.__truediv__.assert_called_once_with("paused_runs")
        self.mock_paused_runs_dir.__truediv__.assert_called_once_with(
            f"paused_run_{self.test_run_id}.json"
        )
        ...
        self.mock_logger.info.assert_any_call(
            f"Deleting paused flow state file for run_id '{self.test_run_id}' at {self.mock_paused_file_path}"
        )
    ...
    def test_delete_paused_flow_state_not_found(self):
        ...
        self.mock_paused_file_path.__str__.return_value = (
            f"mock/path/to/paused_run_{self.test_run_id}.json"
        )

        mock_getter = MagicMock(
            name="mock_get_paused_flow_file_path_in_test"
        )  # Give mock a specific name
        self.sm_for_pause_tests._get_paused_flow_file_path = mock_getter
        mock_getter.return_value = self.mock_paused_file_path

        # Diagnostic assert
        assert (
            self.sm_for_pause_tests._get_paused_flow_file_path is mock_getter
        ), "Mock not assigned correctly!"
        ...
        mock_getter.assert_called_once_with(self.test_run_id)
        self.assertEqual(
            self.mock_paused_file_path.exists.call_count, 2
        )  # Observed behavior is 2 calls
...
class TestStateManagerGetRunId(unittest.TestCase):
    ...
    def test_get_run_id_status_file_error(self):
        ...
        self.mock_logger.error.assert_any_call(
            "Failed to get current run_id due to StatusFileError: Read failed"
        )
...
def test_update_status_existing_run(state_manager_fixture):
    ...
    state_manager.status_file_path.write_text(
