...
The Autonomous Project Engine (APE), as detailed in other architectural documents
(e.g., `system_overview.md`, `detailed_architecture.md` - note: the specific
`autonomous_project_engine_overview.md` may contain outdated architectural details),
operates in major cycles (LOPRD, Blueprint, Plan, Code, Documentation). While internal sub-cycles (e.g., LOPRD refinement via ARCA) aim for full autonomy, the `MasterExecutionPlan` can be designed to pause at the end of major cycles for human review and explicit approval before proceeding.
...
Previously, a conceptual `ProjectChromaManagerAgent_v1` (PCMA) was envisioned for managing artifacts in ChromaDB. However, in the current architecture, individual agents or shared utilities are responsible for storing and retrieving artifacts (like LOPRDs, Blueprints, code) using ChromaDB, often via MCP tools or direct integration.

Therefore, to review artifacts:
*   The document IDs found in `project_status.json` (outputs of stages) correspond to artifacts stored in the project's ChromaDB instance (`dev_chroma_db/` or similar configured path).
*   While a dedicated PCMA agent for querying these is not currently active, you would typically:
    *   Examine agent logs if they output artifact content or summaries.
    *   Inspect `llm_full_response` fields in `project_status.json` for stages that might have logged the full (mocked or actual) content they generated or processed.
    *   For a deeper dive, you might need to query ChromaDB directly using the collection names and document IDs if you know them (e.g., `loprd_artifacts_collection`, `blueprint_artifacts_collection` - though collection names can vary, check agent code or `StateManager` for specifics).
*   The goal is to understand the content of LOPRDs, Blueprints, generated code files, PRAA/RTA reports, and final documentation to inform your review.
