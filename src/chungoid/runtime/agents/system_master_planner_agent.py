...
DEFAULT_MASTER_PLANNER_SYSTEM_PROMPT = (
    ...
    "\\n"
    "1. **Goal Analysis Stage**: Use `NoOpAgent_v1` to analyze and refine the user goal. This stage should extract key requirements, identify the target platform/language, and establish project scope.\\n"
    "2. **Environment Setup Stage**: Use `EnvironmentBootstrapAgent` to set up the development environment based on detected project requirements.\\n"
    "3. **Project Structure Stage**: Use `SystemFileSystemAgent_v1` to create the basic project directory structure and essential files.\\n"
    "\\n"
    "Only after these foundational stages should you proceed with implementation stages using appropriate agents.\\n"
    "\\n"
    ...
    "- `SystemFileSystemAgent_v1` (was FileOperationAgent_v1): Performs file system operations. Requires `tool_name` (str) and `tool_input` (dict) in `inputs`. Example:\n"
    r"    ```\n"
    r"    {\n"
    r"      \"agent_id\": \"SystemFileSystemAgent_v1\",\n"
    r"      \"inputs\": {\n"
    ...
    r"    }\n"
    r"    ```\n"
    r"  - **Important:** Ensure `path` values in `tool_input` are relative to the project root.\n"
    ...
    "- `EnvironmentBootstrapAgent`: Comprehensive environment setup and dependency management. **REQUIRES** `project_path` (str) in `inputs`. Optionally accepts `strategy` (str), `environment_types` (list), `install_dependencies` (bool). Example: `{\"agent_id\": \"EnvironmentBootstrapAgent\", \"inputs\": {\"project_path\": \"{context.data.project_root_path}\"}}`. Handles Python, Node.js, Java environments with intelligent project type detection.\\n"
    "- `NoOpAgent_v1`: Simple pass-through agent for testing and placeholder operations. **REQUIRES** `passthrough_data` (dict) in `inputs`. Example: `{\"agent_id\": \"NoOpAgent_v1\", \"inputs\": {\"passthrough_data\": {\"user_goal\": \"{context.data.user_goal}\"}}}`. Use for requirements gathering, analysis, or planning stages that need simple data flow.\\n"
