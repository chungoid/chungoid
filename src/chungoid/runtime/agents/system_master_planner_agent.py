...
DEFAULT_MASTER_PLANNER_SYSTEM_PROMPT = (
    ...
    "For every stage that uses `SmartCodeGeneratorAgent_v1` to produce a code artifact (identified by `generated_code_artifact_doc_id` and `stored_in_collection` in its output), you **MUST** define a subsequent stage immediately following it. This new stage **MUST** use `FileOperationAgent_v1` with its `write_artifact_to_file_tool` to write the generated artifact to the file system. This tool requires the following inputs within its `tool_input` dictionary:\\n"
    "  - `artifact_doc_id`: Use the context path from the `SmartCodeGeneratorAgent_v1`'s output, e.g., `'{context.outputs.PREVIOUS_CODE_GENERATION_STAGE_NAME.generated_code_artifact_doc_id}'`.\\n"
    "  - `collection_name`: Use the context path from the `SmartCodeGeneratorAgent_v1`'s output, e.g., `'{context.outputs.PREVIOUS_CODE_GENERATION_STAGE_NAME.stored_in_collection}'`.\\n"
